...
from schema import FilterRequest, CreateLabelSerialized
...
def get_feedback_metrics(
    filters: Optional[FilterRequest] = None, index: Optional[str] = None
):
    """
...
def export_feedback(
    ...
    if only_positive_labels:
        ...
        labels = document_store.get_all_labels(
            filters={"origin": ["user-feedback"]}, index=index
        )
    ...
    for label in labels:
        ...
        if full_document_context:
            ...
            additional_context_at_end = max(
                context_to_add - offset_start_in_document, 0
            )
            end_pos = min(
